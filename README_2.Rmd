...
- how important are 100+ yard games?
  # any preds?
- find 90+ yd games
- how does a good defesnse affect nw scoring by postiion?
...
# Load stats
overall <- calculate_full_stats(pbp, weekly = FALSE)
weekly <- calculate_full_stats(pbp, weekly = TRUE)

# Load secondary data
injuries <- read_csv("./raw_data/injury_data.csv")

rosters <- read_csv("./raw_data/rosters.csv")

games <- read_csv("./raw_data/game_data.csv")

adp_historic <- read_csv("./raw_data/adp_historic.csv") %>%
  select(season, player_id, sleeper_id, pick, overall:n)

adp_2021 <- read_csv("./raw_data/adp_2021.csv") %>%
  select(season, player_id, sleeper_id, pick, overall:n)
```

```{r Join secondary data}
# Weekly stats ----
weekly_joined <- weekly %>%
...
overall_joined <- overall_joined %>% mutate(outside_rz_targets = targets - rz_targets)

overall_cleaned <- overall_joined %>%
  select(tidyselect::any_of(
    c(
      # basic info
      "season",
