...
rosters <- read_csv("./data/rosters.csv") %>% mutate(side_of_ball = ifelse(nw_position %in% c("QB", "RB", "WR/TE"), "OFF", "DEF"))
...
average_qbs <-
  ...
  group_by(player_id, season) %>% 
  summarise(across(.cols = c(completions:int_tds, 
                             carries:rushing_tds, inside_ten_carries,
                             nw_pts, fantasy_points),
                   .fns = mean, na.rm = TRUE,
                   .names = "{.col}_per")) %>% 
  mutate(across(where(is.numeric), round, 2))
...
average_rbs <-
  ...
  group_by(player_id, season) %>% 
  summarise(across(.cols = c(carries:receiving_tds, rz_targets,
...
week_1 <- games_21 %>% filter(week == 1) %>% select(location, team, opp)

allowed_20 <- fpts_allowed %>% filter(season == 2020) %>% select(opp = opponent, position, per) %>% 
  arrange(-per) %>% 
  mutate(rk = 1:n())

joined_qbs_week_1 <- overall_qbs3 %>% left_join(week_1, by = c("recent_team" = "team")) %>% left_join(allowed_20, by = c("opp", "position"))
