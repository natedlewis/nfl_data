...
calculate_full_stats <- function(pbp, weekly = TRUE, advanced = FALSE) {
  # Prepare data ------------------------------------------------------------

  # load plays with multiple laterals
  con <-
    url(
      "https://github.com/mrcaseb/nfl-data/blob/master/data/lateral_yards/multiple_lateral_yards.rds?raw=true"
    )
  ...
  if (!"special" %in% names(pbp)) {
    ...
    pbp <- pbp %>%
      dplyr::mutate(special = dplyr::if_else(
  ...
  pass_df <- data %>%
    ...
    dplyr::summarize(
      passing_yards_after_catch = sum((.data$passing_yards - .data$air_yards) * .data$complete_pass,
        na.rm = TRUE
      ),
      ...
      passing_tds = sum(
        .data$touchdown == 1 &
          .data$td_team == .data$posteam & .data$complete_pass == 1
      ),
      ...
      "passing_yards_250" = sum(ifelse(
        passing_yards >= 250 &
          passing_yards < 300, 1, 0
      )),
  ...
  pass_df <- pass_df %>%
    ...
    dplyr::full_join(pass_two_points,
      by = c("player_id", "week", "season", "name_pass", "team_pass")
    ) %>%
    dplyr::mutate(passing_2pt_conversions = dplyr::if_else(
    ...
    dplyr::bind_rows(
      mult_lats %>%
        ...
        dplyr::select(
          "season",
          "week",
          "rusher_player_id" = .data$gsis_player_id,
  ...
  rush_df <- rushes %>%
    ...
    dplyr::mutate(
      ...
      "rushing_yards_90" = ifelse(rushing_yards >= 90 &
        rushing_yards < 100, 1, 0),
  ...
  rush_df <- rush_df %>%
    ...
    dplyr::full_join(rush_two_points,
      by = c("player_id", "week", "season", "name_rush", "team_rush")
    ) %>%
    dplyr::mutate(rushing_2pt_conversions = dplyr::if_else(
  ...
  rec_df <- rec %>%
    ...
    dplyr::mutate(
      ...
      "receiving_yards_90" = ifelse(receiving_yards >= 90 &
        receiving_yards < 100, 1, 0),
  ...
  player_df <- pass_df %>%
    ...
    dplyr::mutate(
      player_name = dplyr::case_when(
        !is.na(.data$name_pass) ~ .data$name_pass, !is.na(.data$name_rush) ~ .data$name_rush, !is.na(.data$name_receiver) ~ .data$name_receiver,
        TRUE ~ .data$name_st
      ),
      recent_team = dplyr::case_when(
        !is.na(.data$team_pass) ~ .data$team_pass, !is.na(.data$team_rush) ~ .data$team_rush, !is.na(.data$team_receiver) ~ .data$team_receiver,
        TRUE ~ .data$team_st
      )
    ) %>%
    dplyr::select(any_of(
    ...
    dplyr::mutate(
      ...
      pass_pts =
        10 * passing_tds_1 +
          ...
          25 * passing_yards_400 + -5 * interceptions + -15 * int_tds +
  ...
  if (isTRUE(advanced)) {
    player_df <- player_df %>% 
      left_join(cleaned_rosters %>% select(-team), by = c("player_id" = "gsis_id", "season")) %>% 
      left_join(raw_games, by = c("season", "week", "recent_team" = "team")) %>% 
      left_join(raw_snaps %>% select(-team_abbr), by = c("player_id", "season", "week"))
  }
  ...
  if (isFALSE(weekly)) {
    player_df <- player_df %>%
      ...
      dplyr::summarise(
        ...
        passing_yards_250 = sum(passing_yards_250),
        ...
        rushing_yards_90 = sum(rushing_yards_90),
        ...
        receiving_yards_90 = sum(receiving_yards_90),
